#!/usr/bin/env bash
#
# issue_watcher.sh — GitHub Issue監視 → TODO.md追記 → auto_dev.sh呼び出し
#
...
main() {
  ...
  # 新規タスクがあり、auto_dev.shが実行中でなければ起動
  if [[ $new_tasks -gt 0 && ! -f "$LOCK_FILE" ]]; then
    log "auto_dev.sh を起動します"
    nohup bash "$SCRIPT_DIR/auto_dev.sh" >> "$LOG_DIR/auto_dev_triggered.log" 2>&1 &
    log "auto_dev.sh を起動しました (PID: $!)"
  elif [[ -f "$LOCK_FILE" ]]; then
    log "auto_dev.sh は既に実行中（スキップ）"
  fi
